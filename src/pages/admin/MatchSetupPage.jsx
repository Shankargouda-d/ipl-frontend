...
export default function MatchSetupPage() {
  ...
  useEffect(() => {
    http.get(`/matches/${id}`).then((r) => {
      setMatch(r.data);
      http
        .get(`/players?team_id=${r.data.team1_id}`)
        .then((p) => setTeam1Players(p.data));
      http
        .get(`/players?team_id=${r.data.team2_id}`)
        .then((p) => setTeam2Players(p.data));
    });
  }, [id]);
  ...
  const saveToss = async () => {
    ...
    try {
      ...
      setMsg("Error saving toss");
    }
    setSaving(false);
  };

  // Allow up to 12 players (11 + 1 impact) in selection
  const toggleSelect = (pid, obj, setObj) => {
    setObj((prev) => {
      const next = { ...prev };
      if (next[pid]) {
        delete next[pid];
      } else if (Object.keys(next).length < 12) {
        next[pid] = true;
      }
      return next;
    });
  ...
  const savePlaying11 = async () => {
    ...
    if (s1.length !== 12) return setMsg("Select exactly 12 players for Team 1");
    if (s2.length !== 12) return setMsg("Select exactly 12 players for Team 2");
    setSaving(true);
    try {
      ...
      await http.post("/squad", {
        ...
        players: s2.map((pid) => ({
          player_id: pid,
          is_impact_player: pid === impact2,
        })),
      });
      navigate(`/admin/matches/${id}/scorecard`);
    } catch {
      setMsg("Error saving playing 11");
    }
    setSaving(false);
  };
    ...
    <div style={{ minHeight: "100vh", background: "#0f0f0f", color: "#fff" }}>
      ...
      >
        <h1 style={{ margin: 0, fontSize: 20, color: "#d85a30" }}>
          Match #{match.match_number} Setup
        </h1>
        <Link
          to="/admin/dashboard"
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 16px" }}>
        ...
        {msg && (
          <p style={{ color: "#e24b4a", textAlign: "center" }}>{msg}</p>
        )}
          ...
          >
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", gap: 12 }}>
                {[
                  { id: match.team1_id, name: match.team1_short },
                  { id: match.team2_id, name: match.team2_short },
                ].map((t) => (
                  <button
                    key={t.id}
        ...
        {/* STEP 2: Playing 11 + Impact (12 players) */}
        {step === 2 && (
          <div>
            <div
              style={{
            ...
            >
              <TeamSelector
                ...
                impact={impact1}
                onToggle={(pid) =>
                  toggleSelect(pid, selected1, setSelected1)
                }
                onImpact={setImpact1}
              />
              <TeamSelector
                ...
                impact={impact2}
                onToggle={(pid) =>
                  toggleSelect(pid, selected2, setSelected2)
                }
                onImpact={setImpact2}
              />
            </div>
            <div style={{ textAlign: "center", marginTop: 28 }}>
              ...
              >
                {saving
                  ? "Saving..."
                  : "Save Playing 11 & Go to Scorecard →"}
              </button>
            </div>
          </div>
        )}
